        ...
        && std::mem::size_of::<T>() == 1
    {
        ...
        let bytes = unsafe {
            std::slice::from_raw_parts(
                ...
                samples.len() * std::mem::size_of::<T>(),
            )
        };
        compress_zlib_libdeflate(bytes, level).map_err(|reason| Error::Other(reason))
    } else {
...
fn compress_zlib_libdeflate(data: &[u8], level: u8) -> std::result::Result<Vec<u8>, String> {
    use libdeflater::{CompressionLvl, Compressor};

    let lvl = CompressionLvl::new(i32::from(level.max(1))).unwrap_or_else(|_| CompressionLvl::best());
    let mut compressor = Compressor::new(lvl);
    let mut output = vec![0u8; compressor.zlib_compress_bound(data.len())];
    let nbytes = compressor
        .zlib_compress(data, &mut output)
        .map_err(|err| format!("libdeflate zlib compress failed: {err}"))?;
...
impl CogBuilder {
    ...
    /// Write a COG using pre-built overview arrays instead of resampling the base image.
    ///
    /// `overviews` must follow `overview_levels` order (e.g. levels `[2, 4, 8]` need
    /// arrays at 1/2, 1/4, and 1/8 resolution).
    pub fn write_layers_3d_to<T: NumericSample, W: Write + Seek>(
        &self,
        mut sink: W,
        data: ArrayView3<T>,
        overviews: &[ArrayView3<T>],
    ) -> Result<()> {
        ...
        if overviews.len() != overview_levels.len() {
            return Err(Error::InvalidConfig(format!(
                "expected {} overview arrays for levels {overview_levels:?}, got {}",
                overview_levels.len(),
                overviews.len()
            )));
        }
        ...
        for (idx, &level) in overview_levels.iter().enumerate() {
            ...
            if oh != expected_h || ow != expected_w || ob != bands {
                return Err(Error::InvalidConfig(format!(
                    "overview level {level} expects shape ({expected_h}, {expected_w}, {bands}), got ({oh}, {ow}, {ob})"
                )));
            }
        }
        ...
        for idx in (0..overview_levels.len()).rev() {
            images[1 + idx].blocks =
                spool_tiled_data_3d(&mut spool, overviews[idx], fill_value, plan)?;
        }
    ...
    /// Write a COG with pre-built overview arrays to a file path.
    pub fn write_layers_3d<T: NumericSample, P: AsRef<Path>>(
        &self,
        path: P,
        data: ArrayView3<T>,
        overviews: &[ArrayView3<T>],
        ...
        let file = File::create(path)?;
        let writer = BufWriter::new(file);
        self.write_layers_3d_to(writer, data, overviews)
    }

    /// Write a planar RGB COG from separate band planes (no chunky interleave copy).
    pub fn write_planar_layers_3d_to<T: NumericSample + Send + Sync, W: Write + Seek>(
        &self,
        mut sink: W,
        width: usize,
        height: usize,
        planes: [&[T]; 3],
        overviews: &[(usize, usize, [&[T]; 3])],
    ) -> Result<()> {
        if planes[0].len() != width * height
            || planes[1].len() != width * height
            || planes[2].len() != width * height
        {
            return Err(Error::InvalidConfig(
                "planar band lengths must equal width * height".into(),
            ));
        }
        ...
        if overviews.len() != overview_levels.len() {
            return Err(Error::InvalidConfig(format!(
                "expected {} overview plane sets for levels {overview_levels:?}, got {}",
                overview_levels.len(),
                overviews.len()
            )));
        }
        ...
        for (idx, &level) in overview_levels.iter().enumerate() {
            ...
            if *oh != expected_h || *ow != expected_w {
                return Err(Error::InvalidConfig(format!(
                    "overview level {level} expects {expected_w}x{expected_h}, got {ow}x{oh}"
                )));
            }
            let expected_len = ow * oh;
            for band in oplanes {
                if band.len() != expected_len {
                    return Err(Error::InvalidConfig(format!(
                        "overview level {level} band length mismatch: expected {expected_len}, got {}",
                        band.len()
                    )));
                }
            }
        ...
        for idx in (0..overview_levels.len()).rev() {
            let (ow, oh, oplanes) = &overviews[idx];
            images[1 + idx].blocks =
                spool_planar_data(&mut spool, *ow, *oh, *oplanes, fill_value, plan)?;
        }

        images[0].blocks =
            spool_planar_data(&mut spool, width, height, planes, fill_value, plan)?;
    ...
    /// Write a planar RGB COG to a file path.
    pub fn write_planar_layers_3d<T: NumericSample + Send + Sync, P: AsRef<Path>>(
        &self,
        path: P,
        width: usize,
        height: usize,
        ...
        let file = File::create(path)?;
        let writer = BufWriter::new(file);
        self.write_planar_layers_3d_to(writer, width, height, planes, overviews)
    }

    /// Begin a planar COG that accepts independently compressed tile blocks per layer.
    pub fn begin_planar_stream<T: NumericSample, W: Write + Seek>(
        &self,
        sink: W,
        width: usize,
        height: usize,
        overview_sizes: &[(usize, usize)],
    ) -> Result<PlanarCogStream<W>> {
        let overview_levels = self.normalized_overview_levels()?;
        if overview_sizes.len() != overview_levels.len() {
            return Err(Error::InvalidConfig(format!(
                "expected {} overview sizes for levels {overview_levels:?}, got {}",
                overview_levels.len(),
                overview_sizes.len()
            )));
        }
        ...
        let prefix = gdal_structural_metadata_bytes(self.inner.planar_configuration);
        let images = self.build_images::<T>(&overview_levels, tw as u32, th as u32)?;
        let encode_plan = LayerEncodePlan {
            tile_width: tw,
            tile_height: th,
            compression: self.inner.compression,
            predictor: self.inner.predictor,
        ...
        Ok(PlanarCogStream {
            sink,
            spool: BlockSpool::new()?,
            images,
            encode_plan,
            layer_block_counts,
            prefix,
            tiff_variant: self.inner.tiff_variant,
            committed,
        })
    }
...
/// raster in memory.
/// Compression parameters for independently encoded COG tile blocks.
#[derive(Debug, Clone, Copy)]
pub struct LayerEncodePlan {
...
/// A compressed planar tile block before it is placed in the COG spool.
#[derive(Debug, Clone)]
pub struct PackedPlanarTile {
...
/// Incremental planar COG writer that accepts pre-compressed tile blocks per layer.
pub struct PlanarCogStream<W: Write + Seek> {
    sink: W,
    spool: BlockSpool,
    images: Vec<CogImage>,
    encode_plan: LayerEncodePlan,
    layer_block_counts: Vec<usize>,
    prefix: Vec<u8>,
    tiff_variant: TiffVariant,
    committed: Vec<bool>,
}

impl<W: Write + Seek> PlanarCogStream<W> {
    pub fn encode_plan(&self) -> LayerEncodePlan {
        self.encode_plan
    }

    pub fn layer_block_count(&self, layer: usize) -> usize {
        self.layer_block_counts[layer]
    }

    pub fn commit_layer(&mut self, layer: usize, packed: Vec<PackedPlanarTile>) -> Result<()> {
        if layer >= self.images.len() {
            return Err(Error::InvalidConfig(format!("invalid COG layer index {layer}")));
        }
        if self.committed[layer] {
            return Err(Error::Other(format!("COG layer {layer} already committed")));
        }

        let total_blocks = self.layer_block_counts[layer];
        let mut blocks = vec![
            CogBlockRecord {
                spool_offset: 0,
        ...
        for tile in packed {
            if tile.block_index >= total_blocks {
                return Err(Error::InvalidConfig(format!(
                    "block index {} out of range for layer with {total_blocks} blocks",
                    tile.block_index
                )));
            }
            blocks[tile.block_index] = if tile.sparse {
                CogBlockRecord {
                    spool_offset: 0,
                    logical_offset_delta: 0,
                    logical_byte_count: 0,
                ...
                self.spool
                    .append_segmented(&tile.leader, &tile.payload, &tile.trailer)?
            };
        }

        self.images[layer].blocks = blocks;
        self.committed[layer] = true;
        Ok(())
    }

    pub fn finish(mut self) -> Result<W> {
        if self.committed.iter().any(|done| !done) {
            return Err(Error::Other(
                "not all COG layers were committed before finish".into(),
            ));
        }
        ...
        let layout = plan_cog_layout(
            base_offset,
            checked_len_u64(self.prefix.len(), "COG prefix")?,
            self.tiff_variant,
            &self.images,
        )?;
        emit_cog(
...
pub fn pack_u8_planar_tile(
    ...
    let packed = pack_cog_block(samples, block_index, encoding, plan.sparse)?;
    Ok(PackedPlanarTile {
        block_index: packed.block_index,
        leader: packed.leader,
...
fn spool_tiled_data_3d<T: NumericSample + Send + Sync>(
    spool: &mut BlockSpool,
    data: ArrayView3<T>,
    fill_value: T,
    plan: TileWritePlan,
    ...
    let packed = pending
        .into_par_iter()
        .map(|pending| pack_cog_block(&pending.samples, pending.block_index, pending.encoding, plan.sparse))
        .collect::<Result<Vec<_>>>()?;

    for packed in packed {
        blocks[packed.block_index] = if packed.sparse {
            packed.record
        } else {
            spool.append_segmented(&packed.leader, &packed.payload, &packed.trailer)?
        };
    }
    ...
    for band in 0..3 {
        ...
        for tile_row in 0..tiles_down {
            for tile_col in 0..tiles_across {
                ...
                let mut tile_data = vec![fill_value; tw * th];
                for row in 0..rows {
                    let src_row = (tile_row * th + row) * width + tile_col * tw;
                    let dst_row = row * tw;
                    tile_data[dst_row..dst_row + cols]
                        .copy_from_slice(&plane[src_row..src_row + cols]);
                }
                pending.push((block_index, tile_data));
            }
        }
    }
...
fn spool_planar_data<T: NumericSample + Send + Sync>(
    ...
    let bands = 3usize;
    let tiles_per_plane = tiles_across * tiles_down;
    let total_blocks = tiles_per_plane * bands;
    let mut blocks = vec![
        CogBlockRecord {
            spool_offset: 0,
            logical_offset_delta: 0,
...
struct PendingCogBlock<T: NumericSample> {
    block_index: usize,
    samples: Vec<T>,
    encoding: CogBlockEncoding,
}

struct PackedCogBlock {
    block_index: usize,
    record: CogBlockRecord,
    leader: Vec<u8>,
    payload: Vec<u8>,
    trailer: Vec<u8>,
    sparse: bool,
...
fn pack_cog_block<T: NumericSample>(
    samples: &[T],
    block_index: usize,
    encoding: CogBlockEncoding,
    sparse: bool,
) -> Result<PackedCogBlock> {
    if sparse && samples.iter().all(|&value| value == T::zero()) {
        return Ok(PackedCogBlock {
            block_index,
            record: CogBlockRecord {
                spool_offset: 0,
                logical_offset_delta: 0,
