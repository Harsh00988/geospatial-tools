...
/// Encode base + overview pyramid layers into any streaming sink (spool or direct COG).
pub fn encode_overview_layers<T, S>(
    input: &GeoTiffFile,
    sink: &mut S,
    width: u32,
    height: u32,
...
where
    ...
    S: OverviewEncodeSink,
{
    let base_blocks = tile_jobs(width, height, tile_size as u32).len();
    let base_writer = sink.begin_overview_layer(base_blocks)?;
    ...
    sink.commit_overview_layer(base_writer)?;
    ...
    for (level_idx, &level) in levels.iter().enumerate() {
        ...
        let layer_writer = sink.begin_overview_layer(block_count)?;
        ...
        sink.commit_overview_layer(layer_writer)?;
        if chain_next {
    ...
    T: TiffSample + geotiff_writer::NumericSample + Copy + Default + Send + Sync + PartialEq,
{
    ...
    let mut sink = crate::encode::sink::LayerBlockSpoolSink(&mut spool);
